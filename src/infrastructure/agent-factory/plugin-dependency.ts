...
const LIST_ARGUMENTS = ["plugin", "list", "--available", "--json"] as const;
...
export async function ensureAgentFactoryPlugin(
  ...
  let records = await listPlugins(runner);
  ...
  records = await listPlugins(runner);
...
async function listPlugins(runner: ProcessRunner): Promise<readonly PluginRecord[]> {
  const result = await invoke(runner, LIST_ARGUMENTS, LIST_TIMEOUT_MS, "Codex 플러그인 목록 조회");
  return parsePluginRecords(result.stdout);
}
...
async function invoke(
  ...
  try {
    const result = await runner(CODEX_COMMAND, arguments_, { timeout, maxBuffer: MAX_OUTPUT_BYTES });
...
function parsePluginRecords(stdout: string): readonly PluginRecord[] {
  let value: unknown;
  try {
  ...
  const records = Array.isArray(value)
    ...
    : isObject(value) && Array.isArray(value.plugins)
      ? value.plugins
      : undefined;
  if (!records) throw new PluginDependencyError("Codex 플러그인 목록 JSON에 plugins 배열이 없습니다.");
